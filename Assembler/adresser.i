...
{
    if (instrs->data) for (InstructionNode* instr = (InstructionNode*) instrs->data, List i = instrs; i->next; i = i->next, instr = (InstructionNode*) i->data) {
        if (!instr->args) continue;
        for (List arg = instr->args; arg->next; arg = arg->next) {
