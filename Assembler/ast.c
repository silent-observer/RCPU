...
void printArg(ArgumentNode arg)
{
    ...
    case MODEABSI:
        if (arg.label)
            printf("(%s, A) ", arg.label);
        else
            printf("(%d, A) ", arg.value);
        break;
    }
}
...
static uint16_t argToString(char *str, const ArgumentNode arg)
{
    ...
    case MODEB:
        return sprintf(str, "B ");
    case MODEC:
        return sprintf(str, "C ");
    case MODEI:
        if (arg.label)
            return sprintf(str, "%s ", arg.label);
        else
            return sprintf(str, "%d ", arg.value);
    case MODEAD:
        sprintf(str, "(A) ");
    case MODEABS:
        if (arg.label)
    ...
    case MODEABSI:
        if (arg.label)
            ...
            return sprintf(str, "(%d, A) ", arg.value);
    }
    return 0;
}
